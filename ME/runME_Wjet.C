...
//# main function
//###################
void runME_Wjet(TString inputDir, TString fileName, TString outputDir, int seed,int SmearLevel,int ncalls,double Error, int Mass, int nev){

  ERRORthreshold=Error;
  // int process=TVar::HWW;
  int process=TVar::Wp_1jet;
  int maxevt=nev;
...
void NeutrinoIntegration(int process,TString inputDir, TString fileName, TString outputDir, int seed, int SmearLevel,int ncalls,int maxevt, int Mass){
  ...
  outFileName.ReplaceAll(".root","_ME_LRWjet.root");
  ...
  for(int ievt=0;ievt<Ntot;ievt++){
	...
	cout <<  dXsecList[processList[j]] << " +- " << dXsecErrList[processList[j]] << " ( " << ratio <<" ) "<<endl;
      ...
      yield_bg = yield[proc_WW][type_]+ yield[proc_HWW160][type_];
      cout<<"bg_yield="<<yield_bg<<"\n";
      
      numer = 1/( ( MCFMXsec[proc_Wpj ] +  MCFMXsec[proc_Wmj ])*acceptance[proc_Wpj ][type_]) * ( dXsecList[proc_Wpj ] + dXsecList[proc_Wmj ]); 
      cout<<" PWjet= "<< 1/( ( MCFMXsec[proc_Wpj ] +  MCFMXsec[proc_Wmj ])*acceptance[proc_Wpj ][type_]) * ( dXsecList[proc_Wpj ] + dXsecList[proc_Wmj ]) << "\n"; 
      ...
      denom += 1/(MCFMXsec[proc_HWW160 ]*acceptance[proc_HWW160 ][type_]) * dXsecList[proc_HWW160 ] * yield[proc_HWW160][type_]/yield_bg;
      cout<<" PHHWW160 = "<< 1/(MCFMXsec[proc_HWW160]*acceptance[proc_HWW160][type_]) * dXsecList[proc_HWW160] * yield[proc_HWW160][type_]/yield_bg <<"\n";	

      if(denom!=0)
	LR[proc_Wpj]=numer/denom;
      cout<<"[LR_Wpj""]= "<<LR[proc_Wpj]<<"\n";
