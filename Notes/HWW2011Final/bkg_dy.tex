...
(among which the tighter $\met$ cuts).  We take the nominal value from
simulation in the last bin before the signal one for the full Higgs
selection, where no further extrapolation is needed except the {\it
in/out} region of the Z-peak.  To gain statistical power, the
$R_{out/in}$ is evaluated without applying the transverse mass
cuts. We also reduce the Z-peak window to just 15~\GeV\ around the
Z-peak to reduce top and multi-boson backgrounds. Full set of plots
used to derive $R_{out/in}$ values can be found in
