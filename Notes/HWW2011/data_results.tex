...
collected data. 
In the $\WW$ control region, the top, Drell-Yan and fake backgrounds are estimated from data, while the 
others are taken from MC. 
Results, reported in Table~\ref{tab:wwEstimResData} and \ref{tab:wwEstimResDataMVA} 
for cut-based and MVA analysis respectively, are consistent with MC expectations.
In the limit calculation we use the results provided by the method for the 0-jet bin analysis, 
while for the 1-jet bin we consider MC predictions and assign an uncertainty of 20\% covering the difference 
with the values obtained from data.
The data-driven procedure is not applied for $m_H>$200 $\GeVcc$ since the $\WW$ control region is expected to be significantly 
contaminated by Higgs events. 
For the 2-jet bin case the data luminosity is too small and all yields are taken from the simulation.
