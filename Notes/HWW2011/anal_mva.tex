In order to make maximal use of the information in the data we have performed 
...
multivariate techniques (like artificial neural networks, likelihood estimators, 
...
large correlation with the previous ones or they do not give a large separation 
...
$\mu e$, where the leptons are sorted by its corresponding $\pt$. The 
signal-to-background ratios for the different final states are different, 
hence it provides an additional discriminant power to add that variable.

\end{itemize}

The training has been carried out depending on the Higgs mass and using the 
corresponding signal samples. Besides the $\WW$ selection requirements, we 
apply an upper loose cut on $\mll$ to enhance the signal-to-background ratio. 
