...
We have tacken into account the following systematic uncertainties:

\begin{itemize}
\item {\it Luminosity:} We usethe official CMS result, which is $4\%$.
...
Here we summarize the systematic uncertainties associated with the methods used.
  ...
  uncertainty on the $gg \to WW$ contribution, which is around $50\%$.
...
while the statistical component is about $50\%$ for $50 \ipb$ and decreases to $20\%$ with $1 \ifb$.
  \item Drell-Yan background: The uncertainty is dominated by the knowledge of
  events with large $\met$ tails. 
The current conservative estimate is $100\%$, which has a small affect on the final analysis results
because very few $\dyll$ events are selected.
...
These uncertainties must be augmented by the luminosity normalisation uncertainty.
  \end{itemize}
...
are summarised in Table~\ref{tab:systww}.
