// @(#)root/tmva $Id: trainMVA_smurf_hzz.C,v 1.2 2011/07/04 06:46:19 ceballos Exp $
/**********************************************************************************
 ...
 )
{
  ...
  if (mvaVar["mtHZZ"])        factory->AddVariable( "mtHZZ",         "MTHZZ(Higgs)",        "GeV", 'F' );
  ...
  if (mvaVar["mtHZZ"])        { cout << "Adding variable to MVA training: mtHZZ"          << endl; nVariablesTemp++; }
  ...
  for (UInt_t i=0; i<signal->GetEntries(); i++) {
    ...
    if( pmet <= 50                       ) continue; // cut on pmet for all lepton-pair flavors
    ...
    double mtHZZ = TMath::Power(sqrt(dilep->pt()*dilep->pt()+91.1876*91.1876)+
    ...
    if (mvaVar["mtHZZ"])         vars[varCounter++] = mtHZZ;
