...
{   
  ...
  for( unsigned int i = 0 ; i < nsamples ; ++i ){
    ...
    if(Use["Likelihood"])  br_likelihood = clone->Branch(Form("likelihood_hww%i_ww" ,mH) , &likelihood , Form("likelihood_hww%i_ww/F" ,mH) );
    ...
    if(Use["likelihood"])  br_likelihood-> SetTitle(Form("Likelihood Output H%i" , mH));
