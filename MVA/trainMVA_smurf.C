// @(#)root/tmva $Id: trainMVA_smurf.C,v 1.3 2011/03/17 18:06:32 benhoob Exp $
/**********************************************************************************
 ...
 )
{
  ...
  for (UInt_t i=0; i<signal->GetEntries(); i++) {
    ...
    if( pmet <= 20	            ) continue; // cut on pmet for all lepton-pair flavors
    if( pmet <= 35 && 
      (type == mm || 
  ...
  if (Use["MLPBNN"])
    factory->BookMethod( TMVA::Types::kMLP, "MLPBNN", "H:!V:NeuronType=tanh:VarTransform=N:NCycles=600:HiddenLayers=N+3:TestRate=5:TrainingMethod=BFGS:UseRegulator" ); // BFGS training with bayesian regulators
  ...
  if (Use["BDT"])  // Adaptive Boost
    factory->BookMethod(TMVA::Types::kBDT,"BDT",
			"!H:!V:NTrees=600:BoostType=AdaBoost:SeparationType=GiniIndex:nCuts=20:PruneMethod=CostComplexity:PruneStrength=20.0");
  ...
  if (Use["BDTD"]) // Decorrelation + Adaptive Boost
    factory->BookMethod(TMVA::Types::kBDT,"BDTD",
                    "!H:!V:NTrees=600:BoostType=AdaBoost:SeparationType=GiniIndex:nCuts=20:PruneMethod=CostComplexity:PruneStrength=20.0:VarTransform=Decorrelate");
