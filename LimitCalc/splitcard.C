...
{ 
  ...
  float nZH(0.0), nWH(0.0), nqqH(0.0), nggH(0.0), nqqWW(0.0), nggWW(0.0), nWjets(0.0), nWgamma(0.0), nZjets(0.0), nVV(0.0), nTop(0.0);
  float sigmanZH(0.0), sigmanWH(0.0), sigmanqqH(0.0), sigmanggH(0.0), sigmanqqWW(0.0), sigmanggWW(0.0), sigmanWjets(0.0), sigmanWgamma(0.0), sigmanZjets(0.0), sigmanVV(0.0), sigmanTop(0.0);
	...
	while ( inputfile.good() )
	  {
	    ...
	    if (TString(line).Contains("Observation", TString::kExact)) 
	      ...
	      newcard << Form("rate  %.3f   %.3f   %.3f   %.3f  %.3f  %.3f   %.3f   %.3f   %.3f  %.3f   %.3f\n", 
			      nZH, nWH, nqqH, nggH, nqqWW, nggWW, nVV, nTop, nZjets, nWjets, nWgamma);
