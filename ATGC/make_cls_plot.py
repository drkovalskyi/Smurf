...
for file in glob(options.input):
    ...
    if f.Get("limit"):
        ...
        f.Close()
        continue     
    if lTree.GetEntries()!=6:
        print "Skipped point with incomplete set of results (file: %s)"%file
        continue
    goodpoint = 1
    for it in range(0,5):
        lTree.GetEntry(it)
        if lTree.GetLeaf("limit").GetValue()<0.001:
            print "Skipped point with zero limit values (file: %s)"%file
            goodpoint = 0
            break
...
text.AddText(0,0.35,"#intL dt= 4.9 fb^{-1}, #sqrt{s} = 7 TeV")
text.Draw()
...
def make1DLimit(limits2d, par, parlatex, parmin, parmax, samples, boundScale, isX):
    ...
    text1d.AddText(0,0.35,"#intL dt= 4.9 fb^{-1}, #sqrt{s} = 7 TeV")
    text1d.Draw()
